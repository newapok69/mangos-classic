...
bool OnGossipSelect(Player* player, Creature* creature, uint32 sender, uint32 action)
{
	...
	case EQUIPMENT_SLOT_END + 8: // Save preset
	{
		...
		if (canSave)
			player->ADD_GOSSIP_ITEM_EXTENDED(GOSSIP_ICON_MONEY_BAG, "Save set", 0, 0, "Insert set name", cost * sTransmogrifier->GetSetCostModifier() + sT->GetSetCopperCost(), true);
